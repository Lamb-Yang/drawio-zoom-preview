...
| `.drawio` 只读文件视图 | ✅ | ✅ | ✅ |
| `.drawio.svg` / `.drawio.png` 双格式嵌入 | ❌（宿主渲染为原生 `<img>`，没有可挂按钮的预览容器；列入计划） | ❌ | ❌ |
...
Drawio 插件 0.7.x 已上线**交互式预览**（预览点击行为选为 Interactive 后，预览自带缩放/平移/Fit/全屏工具栏）。本插件与之共存：

- 交互式预览激活时，本插件的按钮组自动下移，避开宿主工具栏
- 灯箱打开时始终把 viewBox 重置为完整图表——交互模式下缩放/平移过的预览不会被误当成"局部裁剪就是整图"
- 官方交互模式没有的能力仍是本插件的价值：全屏灯箱、多页翻页、用默认应用打开

## 未来

- 支持双格式嵌入（从导出 SVG 的 `content` 属性还原 mxfile，恢复多页翻页）
- 命令面板命令 / 快捷键、灯箱键盘导航（←/→ 翻页、+/− 缩放）
- 按文件记忆上次查看的页码与缩放位置
